...
`analyze`、`crawl`、`batch` 三者的关系可以理解为：

- `analyze`
  负责看清页面结构，判断标题、正文、菜单、噪音区域，以及当前页更像详情页还是目录页。

- `crawl`
  负责验证单个页面是否能被正确抓取，适合在正式批量抓取前确认配置质量。

- `batch`
  负责批量抓取。当页面被判断为目录页或混合页时，可以基于菜单或候选子链接继续抓取整组页面。
...
2. 再执行 `crawl`
   用配置草稿验证单页抓取质量，确认标题、正文和图片是否正确。

3. 最后执行 `batch`
   在确认页面结构和配置可用后，再做整站或整栏目批量抓取。

推荐工作流：

```text
analyze -> 判断页面类型 -> 生成配置草稿 -> crawl 验证单页 -> batch 批量抓取
```
